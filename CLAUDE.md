...
**Phase B (wizards, plan-my-day, inline run exceptions) is still not built.** The roadmap
sequences it before C, but nothing in C depends on it: C adds an event layer beside the
existing screens rather than changing any of the forms B rewrites. Built out of order
deliberately; B is still owed.
