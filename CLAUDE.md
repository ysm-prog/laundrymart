...
- **Deleted on 2026-08-26 at the owner's instruction.** The reasoning above for leaving it alone
  ...
  entry below, so it can be recreated if the evidence is ever wanted back.
...
- **An invoice was generated from it, it proved the roll-up works, and it has since been deleted
  with the rest of the test data** (2026-08-26, at the owner's instruction — see the entry below).
  `INV00001` was a draft of **$55.00** carrying **one line**, raised from `LJ00002`, and for a few
  hours it was the only one of the project's 648 invoices to carry a line at all. So job → price →
  approve → generate **has** been exercised end to end, which §21 had never been able to say — but
  the artefact is gone, and **no invoice on the project carries a line today**. The next run-through
  will be the first against a real customer. *(Superseded twice: this bullet first read "no invoice
  has been generated from any of it… the roll-up is still the one step of the money path never
