...
async function Owed() {
  ...
  return (
    <div className="grid grid-cols-2 divide-x divide-border border border-border lg:grid-cols-4">
      <Stat label="Still owed" value={money(total(owed))} hint={`${owed.length} bills`} />
      <Stat label="Past due" value={money(total(overdue))} hint={`${overdue.length} bills`}
        tone={overdue.length ? "danger" : "default"}
      />
      <Stat label="Owed back to you" value={money(Math.abs(total(credits)))}
        hint={`${credits.length} debit notes`}
      />
      <Stat label="Outstanding documents" value={rows.length} hint="Excludes settled bills" />
    </div>
  );
}
