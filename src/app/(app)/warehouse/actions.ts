...
export async function countReturn(formData: FormData): Promise<void> {
  ...
  if (!route.unloaded_at) {
    fail(BACK, "That van has not been unloaded yet — the driver finishes the run first.");
  }
  ...
  if (already) {
    fail(BACK, `Run ${route.code} was already counted into batch ${already.batch_number}.`);
  }
  ...
  const counted = expected.map((item) => {
    const raw = formData.get(`count_${item.itemId}`);
    const value = Number(raw);
    if (raw == null || raw === "" || !Number.isFinite(value) || value < 0) {
      fail(backTo, `Enter a count for ${item.name} — use 0 if none of it came back.`);
    }
    return { ...item, countedQuantity: Math.trunc(value) };
  });

  if (counted.every((item) => item.countedQuantity === 0)) {
    fail(backTo, "Nothing was counted. If the van really came back empty, leave this run uncounted and tell your supervisor.");
  }
  ...
  const variance = short > 0 || over > 0
    ...
    : "";
  done(`/warehouse/${batch.id}`, `Run ${route.code} counted into batch ${batch.batch_number}.${variance}`);
}
