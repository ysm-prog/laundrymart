...
function crossFieldProblem(input: OrderInput, items: OrderItemInput[]): string | null {
  ...
  if (input.received_via === "driver_pickup" && input.pickup_date && input.pickup_date > input.received_date) {
...
async function resolveDeliveryAddress(supabase: Awaited<ReturnType<typeof createClient>>, input: OrderInput): Promise<string | null> {
  ...
  const { data: location } = await supabase.from("customer_locations")
    .select("address_line1, suburb, state, postcode, is_primary")
    .eq("customer_id", input.customer_id).eq("is_delivery", true).is("deleted_at", null)
    .order("is_primary", { ascending: false }).limit(1)
    .maybeSingle<{ address_line1: string | null; suburb: string | null; state: string | null; postcode: string | null }>();
  ...
  const { data: customer } = await supabase.from("customers")
    ...
    .maybeSingle<{ billing_address_line1: string | null; billing_suburb: string | null; billing_state: string | null; billing_postcode: string | null }>();
...
async function loadOrder(supabase: Awaited<ReturnType<typeof createClient>>, id: string): Promise<{ id: string; order_number: string; status: OrderStatus; delivery_required: boolean; received_at: string } | null> {
  const { data } = await supabase.from("laundry_orders")
    ...
    .eq("id", id).maybeSingle<{ id: string; order_number: string; status: string; delivery_required: boolean; received_at: string }>();
...
export async function createOrder(formData: FormData): Promise<void> {
  ...
  const { data: customer, error: customerError } = await supabase.from("customers")
    .select("id, business_name, depot_id").eq("id", parsed.data.customer_id).is("deleted_at", null)
  ...
  if (!customer) return fail(backTo, "That customer could not be found — please select one from the list.");
  const deliveryAddress = await resolveDeliveryAddress(supabase, parsed.data);
  const { data: orderNumber, error: numberError } = await supabase.rpc("next_number", { t: session.tenantId, k: "laundry_order", p: "LJ" });
  if (numberError) return fail(backToForm, describeDbError(numberError));
  const { data: order, error } = await supabase.from("laundry_orders").insert({
  ...
  if (itemsError) {
    const { error: cleanupError } = await supabase.from("laundry_orders").delete().eq("id", order.id).eq("tenant_id", session.tenantId);
    if (cleanupError) return fail(`/orders/${order.id}`, `Job ${order.order_number} was created but its laundry list could not be saved: ${describeDbError(itemsError)}`);
...
export async function updateOrder(formData: FormData): Promise<void> {
  ...
  const backTo = returnTo(formData, `/orders/${id.data}/edit`);
  const parsed = orderSchema.safeParse(toObject(formData));
  if (!parsed.success) return fail(backWithCustomer(backTo, formData), firstIssue(parsed.error));
  const parsedItems = parseOrderItems(formData.get("items"));
  if (!parsedItems.ok) return fail(backWithCustomer(backTo, formData), parsedItems.problem);
  const items = parsedItems.items;
  const problem = crossFieldProblem(parsed.data, items);
  if (problem) return fail(backWithCustomer(backTo, formData), problem);
  const supabase = await createClient();
  ...
  const customer = await supabase.from("customers").select("id, depot_id").eq("id", parsed.data.customer_id).is("deleted_at", null).maybeSingle<{ id: string; depot_id: string | null }>();
  if (customer.error || !customer.data) return fail(backWithCustomer(backTo, formData), customer.error ? describeDbError(customer.error) : "That customer could not be found.");
  const deliveryAddress = await resolveDeliveryAddress(supabase, parsed.data);
  const row = toRow(parsed.data, deliveryAddress, existing.received_at);
  const { error } = await supabase.from("laundry_orders").update({ ...row, depot_id: session.depotId ?? customer.data.depot_id }).eq("id", id.data).eq("tenant_id", session.tenantId);
  if (error) return fail(backWithCustomer(backTo, formData), describeDbError(error));
  const { error: itemsError } = await supabase.rpc("save_laundry_order_items", { p_order_id: id.data, p_items: items });
  if (itemsError) return fail(`/orders/${id.data}`, describeDbError(itemsError));
  await logOrderActivity(supabase, session, id.data, { activity_type: "updated", previous: { status: existing.status }, next: { ...row, items: items.length } });
  await recordAudit(session, { entity: "laundry_order", entityId: id.data, action: "update", summary: existing.order_number });
  revalidatePath(LIST); revalidatePath(`/orders/${id.data}`);
...
export async function advanceOrder(formData: FormData): Promise<void> {
  ...
  if (target === "completed" || target === "cancelled") return fail(detail, "Use the delivery, collection or cancel action for that step.");
  if (target === "assigned") return fail(detail, "Use Assign Driver to give this job to a driver and a delivery date.");
  ...
  const { error } = await supabase.from("laundry_orders").update({ status: target }).eq("id", parsed.data.id).eq("tenant_id", session.tenantId);
  ...
  revalidatePath(LIST); revalidatePath(detail);
...
export async function completeOrder(formData: FormData): Promise<void> {
  ...
  const existing = await loadOrder(supabase, parsed.data.id);
  if (!existing) return fail(LIST, "That job could not be found.");
  const result = await completeLaundryOrder(supabase, session, existing, { at: toInstant(parsed.data.completed_date, parsed.data.completed_time), handledBy: parsed.data.handled_by, note: parsed.data.note ?? null });
  if (!result.ok) return fail(detail, result.error);
  const { delivered } = result;
  await recordAudit(session, { entity: "laundry_order", entityId: parsed.data.id, action: "status_change", summary: `${existing.order_number} ${delivered ? "delivered" : "collected"}` });
  revalidatePath(LIST); revalidatePath(detail); revalidatePath("/awaiting-invoice");
  return done(detail, `Job ${existing.order_number} ${delivered ? "delivered" : "collected"} and completed; it is now awaiting invoice review.`);
}

export async function cancelOrder(formData: FormData): Promise<void> {
  const session = await assertCapability("orders.manage");
  const parsed = z.object({ id: z.string().uuid(), cancellation_reason: optionalText }).safeParse(toObject(formData));
  if (!parsed.success) return fail(LIST, firstIssue(parsed.error));
  const detail = `/orders/${parsed.data.id}`;
  const supabase = await createClient();
  const existing = await loadOrder(supabase, parsed.data.id);
  ...
  if (!allowed.ok) return fail(detail, allowed.reason);
  const { error } = await supabase.from("laundry_orders").update({ status: "cancelled", cancelled_at: new Date().toISOString(), cancellation_reason: parsed.data.cancellation_reason ?? null, billing_status: "cancelled_no_invoice" }).eq("id", parsed.data.id).eq("tenant_id", session.tenantId);
  if (error) return fail(detail, describeDbError(error));
  await logOrderActivity(supabase, session, parsed.data.id, { activity_type: "cancelled", previous: { status: existing.status }, next: { status: "cancelled" }, note: parsed.data.cancellation_reason ?? null });
  await recordAudit(session, { entity: "laundry_order", entityId: parsed.data.id, action: "status_change", summary: `${existing.order_number} cancelled` });
  revalidatePath(LIST); revalidatePath(detail); revalidatePath("/awaiting-invoice");
...
export async function assignOrder(formData: FormData): Promise<void> {
  const session = await assertCapability("orders.write");
  const parsed = z.object({ id: z.string().uuid(), assigned_to: optionalUuid }).safeParse(toObject(formData));
  if (!parsed.success) return fail(LIST, firstIssue(parsed.error));
  const detail = `/orders/${parsed.data.id}`;
  const supabase = await createClient();
  const existing = await loadOrder(supabase, parsed.data.id);
  if (!existing) return fail(LIST, "That job could not be found.");
  if (existing.status === "cancelled") return fail(detail, "This job was cancelled, so it cannot be assigned.");
  ...
  const { error } = await supabase.from("laundry_orders").update({ assigned_to: parsed.data.assigned_to ?? null }).eq("id", parsed.data.id).eq("tenant_id", session.tenantId);
  if (error) return fail(detail, describeDbError(error));
