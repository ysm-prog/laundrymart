/**
 * Giving a Job to a Board for a day, with no database in sight.
 *
 * Four vocabularies meet in this feature and it is worth naming them once,
 * because the words and the tables do not line up:
 *
 *   - a **Job**   is `laundry_orders` — a customer's laundry, counter to hand-back;
 *   - a **Board** is `boards` — a standing delivery round with its own login;
 *   - a **Run**   is `daily_routes` — one board, one day;
 *   - a **Stop**  is `public.jobs` — one visit to one customer on that run.
 *
 * The **user-facing model is only the first two**: `Job → Board → Delivery
 * Date`, stored as `assigned_board_id` + `assigned_delivery_date` on the job
 * itself (0016, retargeted by 0031). Neither office staff nor rounds ever name a
 ...
 *
 * **A driver is a person and is deliberately not the assignment target.** Who
 * operated a round on a given day is recorded on the run
 * (`daily_routes.operated_by_driver_id`), so cover, sickness and turnover cost
 * one field rather than a reassignment sweep — which is the whole reason 0031
 * exists.
 *
 ...
 * `guard_laundry_order_transition()` in migrations 0016/0031, whose rules are a
...
/**
 * The status a job must reach before it can be given to a board.
 *
 * Not `out_for_delivery`: that means it has already physically left, so it has a
 * board by definition and what it needs is reassignment, not assignment. Not
...
/**
 * The statuses My Runs shows a board for a day.
 *
 ...
 */
export const RUN_DAY_STATUSES: readonly OrderStatus[] = [
...
/**
 * May this job be given to a board — and if not, why, in a sentence the person
 * looking at it can act on?
 *
 * `allowAssigned` is the difference between assigning and reassigning: the
 * dialog offering a fresh assignment refuses a job that already has a board,
...
export function checkAssignable(
  ...
  if (!options.allowAssigned && job.assigned_board_id) {
    return { ok: false, reason: "This job is already assigned to another board." };
  }
  return { ok: true };
}
 ...
 * round is holding it — so past that point reassignment is the tool, and it is
 * a management override.
 */
export function checkAssignmentRemovable(
  job: AssignableJob,
): { ok: true } | { ok: false; reason: string } {
  if (!job.assigned_board_id) {
    return { ok: false, reason: "This job is not assigned to a board." };
  }
  if (job.status !== "assigned") {
    return {
      ok: false,
      reason: job.status === "out_for_delivery"
        ? "This job is already out for delivery. Reassign it instead of removing the board."
...
export function isUnassignedDeliveryWork(job: AssignableJob): boolean {
  return job.delivery_required && !job.assigned_board_id && job.status === ASSIGNABLE_STATUS;
}
...
export type RunDayJob = { status: OrderStatus | string; load_confirmed_at?: string | null };

export type RunDay<T extends RunDayJob> = {
 ...
 */
export function groupRunDay<T extends RunDayJob>(jobs: readonly T[]): RunDay<T> {
 ...
 */
export function loadableJobs<T extends RunDayJob>(jobs: readonly T[]): T[] {
 ...
 */
export function dispatchableJobs<T extends RunDayJob>(jobs: readonly T[]): T[] {
  return jobs.filter((job) => job.status === "assigned" && !!job.load_confirmed_at);
}

export function checkConfirmLoad<T extends RunDayJob>(
  jobs: readonly T[],
...
export function checkStartRoute<T extends RunDayJob>(
  jobs: readonly T[],
): { ok: true; jobs: T[] } | { ok: false; reason: string } {
