...
**NOT applied to `laundrymart-syd` and not opened behind the auth gate** (no credentials here).
**Apply `0040` before merging** — the schema leads the code, as every release since 2026-08-18.
Then: take a job in, approve it, check Money › Open drafts; take a second job in for the same
customer, approve it, confirm the *same* invoice number picks it up with quantities added rather
than a second line; press Issue now and check it is dated today.
