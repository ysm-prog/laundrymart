-- ============================================================================
-- 0020_return_count — the depot count is the control, not a re-typing exercise
--
-- Renumbered from 0012 on the way in. It was written against a `Prod` that
-- stopped at 0011, but 0012_optional_inspection took that number first and
-- 0013–0019 have shipped since. The number is only an ordering, and nothing
-- here depends on running before them: it adds to the warehouse tables from
-- 0009 and touches nothing 0013+ introduced. It deliberately adds no
-- `grant execute … to anon` (CLAUDE.md §7), which is what makes it safe to
-- place after 0019's revoke rather than before it.
--
